...
///
/// The methods take the locked scheduler, so a caller can only reach a `&mut WaitQueue` while it
/// holds the lock: two cores never alias the queue (backport of upstream esp-hal#6258).
