...
- SPI slave: SPI slave support for ESP32-P4 and ESP32-S31 (#6181)
- SPI slave: SPI slave DMA support for ESP32-C5 and ESP32-C61 (#6181)
- RMT support for ESP32-P4 and ESP32-S31 (#6174)
- ADC: async ADC API for ESP32-S2 and ESP32-S3 (#6167)
- ADC: ADC calibration data v2 for ESP32-C6 (#6166)
- ADC: Line-fitting calibration schemes for ESP32 and ESP32-S2 (#6166)
- Enabled ADC for ESP32-P4 and ESP32-S31. The ESP32-S31 ADC is differential, but only one pin in the pair is configured. This means the output range is currently ~2100 - ~4300. (#6164)
- PSRAM support for ESP32-S31 (#6155)
- Configurable PSRAM clock source for ESP32-P4 (#6151)
- Crypto: Implemented digest v0.11 traits for `Sha` types (#6143)
- `FROM_CPU_INTRn` peripheral singletons, `SoftwareInterrupt::new`, `interrupt::software::Instance` (#6142)
- Configure the minimum main stack size with `ESP_HAL_CONFIG_ENSURE_MAIN_STACK_MINIMUM`. The linker will assert that there is at least this much stack available. Default: 9182 bytes. (#6139)
- SDMMC support for ESP32-S31 (#6132)
- ADC support for ESP32-C61 (#6130)
- UART: UART DMA (UHCI) support for ESP32-P4 and ESP32-S31 (#6125)
- LP_I2C support for ESP32-C5 and ESP32-P4 (#6118)
- GPIO: `{Input, Output, Flex}::{set_pad_hold, is_pad_held}` (#6100)
- Dedicated GPIO support for ESP32-P4 and ESP32-S31 (#6095)
- `ESP_HAL_CONFIG_USE_XTAL32K` opts in to external 32 kHz crystal (powers it, defaults slow clock, removes crystal pads from `Peripherals`) (#6075)
- Added missing UART2 instance to ESP32-C61 (#6062)
- `LowPower::set_wakeup_deadline` and `LowPower::clear_wakeup_deadline`, which arm the sleep (#6060)
- `LowPower::sleep_deep_with_rejection`, which returns when the hardware rejects the sleep. (#6060)
- `Input::apply_wakeup_config` and `Flex::apply_wakeup_config`, which let a pin wake the chip (#6060)
- `Input::caused_wakeup` and `Flex::caused_wakeup`, which report whether the pin ended the (#6060)
- `UlpCore::enable_wakeup` and `UlpCore::disable_wakeup` on esp32s2 and esp32s3, with (#6060)
- `Uart::enable_wakeup`, `Uart::disable_wakeup`, `UartRx::enable_wakeup` and (#6060)
- The per-pin low-power deep-sleep wakeup path is now available on esp32, esp32c5, esp32c6, (#6060)
- `LpI2c::{read, write}` functions and `Drop` implementation (#6043)
- `into_lp*` functions to `gpio::{Input, Output}` drivers. (#6009)
- PDM support for I2S1/I2S2 where available (#5994)
- Crypto: Enabled AES, SHA, RSA, ECC drivers for ESP32-S31 (#5978)
- basic RNG support for ESP32-S31 (#5976)
- I2C support for ESP32-S31 (#5959)
- SPI master (SPI2/SPI3) and AXI GDMA support for ESP32-S31 (#5958)
- UART support for ESP32S31 (#5957)
- USB: Added USB_HS support for ESP32-S31. (#5938)
- GPIO support for ESP32-S31 (#5936)
- Sleep: ESP32-P4 UART wakeup sources (#5931)
- LP-core wakeup support for ESP32-C5, ESP32-C61, and ESP32-H2. (#5927)
- `RtcPin::rtc_number` on all supported chips. (#5927)
- `at_least_version` macro that allows conditional compilation based on esp-hal version. (#5920)
- LP GPIO: Added ESP32-C5 and ESP32-C61 LP_GPIO support (#5913)
- LP GPIO: Added "Ext1" wakeup source to ESP32-C5 and ESP32-C61 (#5913)
- LP GPIO: ESP32-H2 implementation of `RtcPinWithResistors` now has pullup/pulldown resistor control methods (#5913)
- Ext1WakeupSource and LP GPIO support for ESP32-P4 (#5901)
- LP GPIO: Enabled `lp_io` for ESP32 and ESP32-H2 (#5898)
- SPI: `Spi::half_duplex_read_async` and `Spi::half_duplex_write_async` (#5897)
- I2S: I2S1 and I2S2 peripheral instances on ESP32-P4 (TDM only) (#5890)
- `WakeupSource`, an enum modelling a single wakeup source/trigger bit. It is generated from chip metadata and shared by `WakeTriggers` and `WakeupReason`. (#5878)
- `ESP_HAL_CONFIG_INIT_STACK_PTR_RANGE_CHECK` (default `true`) for platforms that call `esp_hal::init` from a thread. (#5867)
- `#[ram(unstable(dcache_reclaimed))]` to allow using memory not taken up by data cache (#5860)
- P4: I2S0 support (#5847)
- `LowPower` to control sleep (#5839)
- Swd is now available on ESP32-S2 and ESP32-P4 (#5838)
- I2S driver: Basic PDM (pulse-density modulation) support on I2S0 for supported chips, including simplex TX and/or RX with hardware PCM conversion where available. (#5832)
- I2S driver: `I2s::new_pdm`, `i2s::pdm` configuration types (`PdmConfig`, `PdmTxConfig`, `PdmRxConfig`, etc.), `PdmError`, and pin helpers `with_clk`, `with_din_line`, and `with_dout2`. (#5832)
- `Display` and `Error` impls for `DmaBufError` and `DmaAlignmentError` (#5815)
- Sleep: ESP32-P4: support for light/deep sleep, with timer and GPIO (light sleep only) wakeup (#5804)
- `time::Duration` now implements `Default`, returning `Duration::ZERO`. (#5802)
- Sleep: ESP32-C5, ESP32-C61: support for light/deep sleep, with timer and GPIO (light sleep only) wakeup (#5800)
- DMA: DmaAlignedRef counterpart for DmaAlignedMut (#5791)
- Sleep: auto lightsleep for ESP32, ESP32-C2, ESP32-C3, ESP32-S2, ESP32-S3 (#5768)
- ESP32-S3 compile time validation that a 16B cache line size is only used with a supported cache size, matching ESP-IDF. (#5765)
- ESP32, ESP32-S3, ESP32-P4: initial SDMMC/SDIO host driver (#5760)
- i2c: `force_scl_low()` and `force_sda_low()` for active line control via `pd_en` (bus recovery, clock stretching) on all chips except ESP32 (#5757)
- Sleep: `WakeLock`, an RAII guard in `rtc_cntl` that prevents automatic light sleep while held. (#5756)
- SPI: `SpiDma::half_duplex_read_async` and `SpiDma::half_duplex_write_async` (#5754)
- DMA: `esp_hal::dma::aligned::{InternalMemory, DmaAlignedMut}` types. (#5744)
- Option to use PMP to mark DATA and STACK as non-executable, RWTEXT as not writable (not ESP32-C2/C3) - enabled by default (#5722)
- timer: async API to PeriodicTimer (#5712)
- Callback-free I2C slave (target) mode driver for ESP32-C3 and ESP32-C6. This includes SCL clock-stretching flow control and automatic FSM/TX FIFO resets to safely handle mid-transaction master read aborts. (#5709)
- `ESP_HAL_CONFIG_CLEAR_CRYPTO_SECRETS` option (enabled by default) that clears operands and intermediate values from crypto peripheral registers after each operation (#5701)
- Enabled SHA, AES and RSA drivers for ESP32-P4 (#5697)
- DMA: ESP32-S2: DMA_CRYPTO can now be used to drive memory-to-memory transfers. Supports transfers to/from PSRAM. (#5676)
- GPIO: `InputConfig::with_input_sync` allows configuring the input synchronization (#5669)
- DMA: Memory-to-memory DMA across multiple DMA engines (type-erased channels); ESP32-P4 AXI GDMA channels are now usable for mem2mem. (#5667)
- DMA: `Mem2MemCapableChannel` trait for channels marked `mem2mem = true` in device metadata. (#5667)
- SPI: clock source is now configurable (#5660)
- Added `stop` method to `I2sTxDmaTransfer` and `I2sRxDmaTransfer` to immediately abort transfers and return peripheral/buffers. (#5659)
- GPIO: added the `output_level`, `output_levels` and `output_levels_ll` dedicated GPIO function for ESP32-S3 (#5651)
- I2C: `Config::with_scl_sample_level` that allows selecting the SCL level at which SDA is sampled (#5647)
- I2C: `Config::with_bus_arbitration` that allows enabling bus arbitration detection (#5647)
- UART: `Uart::is_break_detected`, `UartRx::is_break_detected`, `Uart::clear_break_detected`, and  `UartRx::clear_break_detected` for RX break detection. (#5640)
- DMA: `DmaEligiblePeripheral` trait, implemented for every DMA-eligible peripheral singleton. (#5625)
- SDM Driver (#5610)
- I2C:  clock source configuration via `ClockSource` and `Config::with_clock_source` (#5606)
- I2S: I2sRx::read and I2sTx::write take ownership of the channel and a DmaRxBuffer / DmaTxBuffer, returning I2sRxDmaTransfer / I2sTxDmaTransfer. (#5603)
- I2S: dma_tx_stream_buffer! macro for statically allocated streaming TX buffers (matching the existing dma_rx_stream_buffer!). (#5603)
- I2S: dma_tx_stream_buffer! macro. (#5603)
- Initial MIPI-DSI support for ESP32-P4 (#5596)
- Ethernet: Public MdioBus trait abstracting Clause-22 MDIO read/write, enabling external PHY drivers and host-side mocks. (#5590)
- UART: New `split_mut` method for borrowing the Uart Rx and Tx halves. (#5586)
- RMT: expose the configured counter clock frequency from `Rmt::frequency` and update the clock tree when configuring the source clock (#5568)
- USB: USB_HS support for ESP32-P4 (#5560)
- ESP32-P4: USB-FS implementation (#5558)
- Ethernet: Basic support for ESP32-P4 (#5556)
- Ethernet: Basic support for ESP32 (#5546)
...
- Documentation: `PeripheralInput/PeripheralOutput` now list what GPIOs implement them, instead of showing a blanket implementation. (#6105)
- `Ext1WakeupSource::new` and `RtcioWakeupSource::new` take `&mut dyn LpPin` on every chip. (#6073)
- `LowPower::sleep_deep` takes an `RtcSleepConfig` instead of a list of wakeup sources, and (#6060)
- A listening pin wakes the chip from light sleep, so `Input::listen`, `Input::wait_for` and (#6060)
- The wakeup-enable mask is cleared during `esp_hal::init`, so a program starts with no (#6060)
- TWAI: `RawFrame` has been removed in favour of `EspTwaiFrame` (#6052)
- `rtc_i2c::I2c` has been renamed to `lp_i2c::LpI2c` (#6043)
- `RtcPin(WithResistors)` have been renamed to `LpPin(WithResistors)` (#6006)
- `LpI2c::new` and `LpUart::new` take pin singletons, not `LowPowerOutput(OpenDrain)` instances. (#5993)
- SPI master: Clock divider calculation has been greatly sped up in some cases. (#5988)
- GPIO wakeup APIs now use `gpio::Level`; removed `sleep::WakeupLevel`. (#5927)
- LP GPIO: The compiler can now infer the `PIN` const generic in the LP GPIO drivers. (#5917)
- LP GPIO: ESP32-S2, ESP32-S3: The `rtc_io` module has been renamed to `lp_io`. (#5898)
- SPI: half-duplex SPI can now transfer more than a FIFO's worth of data (#5896)
- GPIO: `Io::{set_interrupt_priority, set_interrupt_handler}` now affect all cores. (#5886)
- GPIO: `Io::set_interrupt_priority` now enables the GPIO interrupts if they weren't before. (#5886)
- `WakeupReason` is now a set-like wrapper over `WakeupSource` (with `is_empty`, `contains`, and `iter`) instead of a `bitflags` type. (#5878)
- `WakeTriggers` is now a set-like wrapper over `WakeupSource` (with `insert` and `contains`) instead of a per-chip `bitfield`. (#5878)
- shortened clock calibration periods (#5848)
- RTC_FAST is now calibrated on startup and before entering deep sleep (#5848)
- DMA stream buffer views: `DmaRxStreamBufView::{available_bytes, peek, peek_until_eof}` and `DmaTxStreamBufView::available_bytes` now take `&mut self` (#5847)
- `light_sleep` and `deep_sleep` have been removed from `Rtc` (#5839)
- `RTC_TIMER` has been split off from the `LPWR` singleton. (#5839)
- SPI: The SPI master drivers no longer hold the function clock active clock while idle. (#5836)
- SPI: SPI transfer function now flush automatically. `flush` has become a no-op. (#5836)
- I2C: The I2C driver no longer holds its clock active clock while idle. (#5834)
- I2S driver: TDM configuration moved from `Config` to `TdmConfig`; `I2s::new` takes `TdmConfig` directly. The internal `Config` enum is no longer part of the public API. (#5832)
- I2S driver: PDM configuration errors are nested as `ConfigError::Pdm(PdmError)` instead of flat `ConfigError` variants. (#5832)
- ESP32-S2/S3: the `ulp_core` module has been renamed to `lp_core`. (#5828)
- SPI master: the `min_async_transfer_size` config option now also applies to async half-duplex DMA transfers, branching to blocking/CPU driven mode for small transfers that fit in the FIFO. (#5823)
- Sleep: Drop the support for sub-v3.0 ESP32 chips (#5812)
- `wakeup_cause` now returns a `WakeupReason` set of all sources that ended the sleep (#5811)
- Sleep: C6: light sleep powers down XTAL + RC_FAST and drops the HP rail to 0.6 (#5803)
- Sleep: H2: the oscillators are powered down in light sleep (#5803)
- Sleep: S2: brought to parity with C3/S3 (bias/regulator into sleep) (#5803)
- Sleep: ESP32, H2 and S3 now power down more of the chip when only Ext1/LPIO wakeup is used (#5803)
- `TimerWakeupSource::new` now takes `esp_hal::time::Duration` instead of `core::time::Duration`. (#5802)
- `Config::with_timeout` (RTC_I2C) now takes `esp_hal::time::Duration` instead of `core::time::Duration`. (#5802)
- ESP32-S3 default `ESP_HAL_CONFIG_DATA_CACHE_SIZE` is now `64KB` (was `32KB`). The linker already reserves the full 64KB data cache window, so the old default left 32KB of RAM unused. (#5765)
- updated defmt to 1.1 (#5752)
- DMA: DMA buffer types now take `DmaAlignedMut`-wrapped descriptor lists and buffers. (#5744)
- Reverted accidental stabilization of `dma_*` macros. (#5743)
- Skip slow clock calibration for XTAL32K clock source, assume 32.768 kHz instead (#5693)
- DMA: `Mem2Mem::new` takes only a capable channel on chips without `mem2mem_requires_peripheral`; peripheral selector comes from metadata. (#5667)
- Exceptions: The exception messages now include which core has the exception, and what the raw code value is. (#5639)
- DMA: DMA erased channel types have been renamed to drop the `Any` prefix: (#5625)
- I2S: I2S DMA transfers now use the shared DmaBuffer API instead of manually managed descriptor chains. Streaming transfers use dma_rx_stream_buffer! / dma_tx_stream_buffer!; one-shot transfers use (#5603)
- I2S: I2sRxCreator::build and I2sTxCreator::build no longer take a descriptor slice. (#5603)
- I2S: On streaming transfers, available() is now available_bytes() and pop() / push() return usize instead of Result. (#5603)
- ESP32-P4: USB0 singletons have been renamed to USB_FS (#5558)
...
- Partially pre-filled `TxStreamBuf` is now supported and doesn't panic during the transfer (#6161)
- UART: UART clocks are now configured only once, and enough time is left for them to stabilise. This fixes a problem with sending unexpected and possibly incorrect data in some cases. (#6137)
- Order of register arguments to the assembly instructions for dedicated gpio on the esp32-s2 and esp32-s3 (#6122)
- I2C: I2C on the ESP32-P4 now applies its configuration and writes the transmit FIFO through the correct register. (#6108)
- SPI: ESP32 no longer hangs if the transfer finishes while we are in the process of enabling interrupts. (#6107)
- Documentation: `PeripheralInput/PeripheralOutput` implementations no longer show up as unstable for GPIOs. (#6105)
- USB Serial/JTAG: Prevented data loss and stalls during concurrent asynchronous USB Serial/JTAG transmission and reception. (#6104)
- UART on P4 (#6101)
- GPIO: Initializing a GPIO pin now releases the pad hold (#6100)
- USB Serial/JTAG: USB-serial: `flush_tx_async` did not work when the last sent packet was exactly 64 bytes (#6097)
- USB-Serial-JTAG ISR should not clear both interrupt latches unconditionally (#6089)
- `set_analog` and the touch pad setup no longer clear the open-drain bit of another pad on (#6073)
- TWAI transmit async hanging on an "empty" bus (#6070)
- `embassy` GPIO waits can survive an automatic light sleep. `listen` used to clear the pin's (#6060)
- esp32, esp32s2 and esp32s3 no longer leave an `ext1` pad held after a light sleep, which (#6060)
- esp32, esp32c2, esp32c3, esp32s2 and esp32s3 release the wake pads after a deep-sleep wake. (#6060)
- The pad release after a deep-sleep wake no longer reassigns pad functions, which could steal (#6060)
- The digital pads are isolated on every deep sleep on esp32, esp32s2, esp32s3, esp32c2 and (#6060)
- SPI master: SPI clock calculation no longer overshoots the requested frequency. (#5988)
- The I2S drivers now pick the closest representable MCLK divider, so sample rates that do not divide the source clock evenly are reproduced more accurately. (#5969)
- `I2sParallel` generated an incorrect fractional MCLK divider (and therefore a wrong output rate) for sample rates that could not be represented exactly. (#5969)
- `mcpwm` `timer_clock_with_frequency` returns `FrequencyError` instead of panicking when the requested frequency is too high for the timer clock. (#5940)
- LP GPIO: ESP32-S3 no longer panics when trying to construct `LowPower*` drivers with a high pin number. (#5898)
- Rwdt and Swd are no longer constructible outside of esp-hal (#5838)
- RSA: ESP32 no longer truncates the result of RSA operations on large numbers (#5831)
- `wakeup_cause` now reports the timer wakeup correctly on ESP32-C5 and C61 (#5811)
- Sleep: going to light sleep now flushes or suspends UART output (#5792)
- Sleep: fixed sleep configuration of ESP32-C2 and C3 to prevent unexpected resets while sleeping (#5780)
- Sleep: the system time is now corrected by the duration of the light sleep (#5778)
- Sleep: RTC timer correctly provides the up to date timestamp (#5778)
- Sleep: fixed sleep configuration of ESP32-S3 to prevent unexpected resets while sleeping (#5777)
- Systimer: `SystemTimer::set_unit_value` correctly loads the upper bits of the requested value. (#5772)
- UART RX error reporting and erroneous-byte discarding are now configurable. (#5734)
- Keep UART0 source clock enabled before reset and sleep on affected chips. (#5711)
- DMA: Fixed an issue that allowed invoking `dma_*` macros unsoundly (#5688)
- timer: `OneShotTimer::into_blocking()` now returns `OneShotTimer<'d, Blocking>` (#5686)
- Sleep: ESP32-C6, H2: esp-hal no longer powers down the XTAL32K clock source when in use (#5675)
- UART: `wait_for_break` no longer enables the RX break interrupt while polling. (#5640)
- ESP32/ESP32-S2 I2S TX not stopping once the DMA buffer is exhausted (#5628)
- ESP32 sub-sequent TX transactions might not transfer anything (#5628)
- Corrected support status of SDMMC, SDIO and ULP_FSM (#5597)
- ESP32 should no longer break its debug interface when running embedded tests. (#5594)
- SHA: ESP32 implementation no longer powers down the SHA accelerator while in use (#5592)
- SHA: ESP32 no longer tries to use the hardware accelerator when it's held by another SHA operation. (#5592)
- RSA: ESP32 now correctly disables its interrups before disabling the RSA accelerator (#5592)
- Add missing import when building SPI with `ESP_HAL_CONFIG_PLACE_SPI_MASTER_DRIVER_IN_RAM = "true"` (#5585)
- Xtensa LLD can now link projects again, as long as they don't have GCC linked libraries. Support for linking via LLD with GCC binaries will be possible once https://github.com/espressif/llvm-project/pull/130 lands and is included in the esp toolchain. (#5582)
...
- `SoftwareInterruptControl`, `SoftwareInterrupt::steal`, `SW_INTERRUPT` peripheral singleton. (#6142)
- `gpio::LpPinWithResistors`. `gpio::LpPin` now covers every low-power pad, and the pull (#6073)
- The `WakeSource` trait, `WakeTriggers`, `Ext0WakeupSource`, `Ext1WakeupSource`, (#6060)
- `LowPower::sleep`. Use `sleep_light`, `sleep_deep` or `sleep_deep_with_rejection`. (#6060)
- `gpio::WakeEvent`, `gpio::WaitForOptions`, `Input::wakeup_enable`, `Flex::wakeup_enable`, (#6060)
- `LowPowerInput`, `LowPowerOutput` and `LowPowerOutputOpenDrain` are no longer available on chips without a supported LP core. (#6009)
- Unsupported `WakeupSource` variants. (#5927)
- The `WakeupReason::NoSleep` flag and the per-chip `WakeTriggers` bitfield accessors (`set_gpio`, `set_timer`, `set_uart0`, …). (#5878)
- ESP32-S3: Removed the 16kB data cache size option (#5860)
- removed the unused `rtc_cntl::sleep::Error` type (#5838)
- `SleepSource` (the wakeup cause is now reported via the `WakeupReason` flags returned by `wakeup_cause`) (#5811)
- `dma_circular_buffers!`, `dma_circular_descriptors!`, `dma_circular_buffers_chunk_size!`, and `dma_circular_descriptors_chunk_size!` macros. (#5743)
- the `esp_hal::dma::{Word, ReadBuffer, WriteBuffer}` traits have been removed. (#5740)
- DMA: Virtual `MEM2MEM*` peripheral singletons; use a mem2mem-capable DMA channel instead. (#5667)
- DMA: `DmaChannelConvert` trait and its `degrade()` method. Use the `From`/`Into` impls (#5625)
- DMA: `DmaEligible` trait and the associated `PeripheralDmaChannel`, `PeripheralRxChannel`, (#5625)
- DMA: `DmaChannelFor`, `RxChannelFor`, `TxChannelFor` traits. (#5625)
- I2S: I2sRx::read_dma, read_dma_circular, read_dma_circular_async, read_words, and I2sTx::write_dma, write_dma_circular, write_dma_circular_async, write_words. (#5603)
- I2S: AcceptedWord trait. (#5603)
- I2S: Generic DMA transfer types DmaTransferRx, DmaTransferTx, DmaTransferRxCircular, DmaTransferTxCircular, and DescriptorChain. (#5603)
- I2S: DmaError::Late (overrun/underrun is now handled via buffer back-pressure; drain promptly using available_bytes() / pop()). (#5603)
- USB: removed blocking USB support (#5574)
